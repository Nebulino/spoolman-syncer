...
3. Post-Processing: Sanitizes filenames, fixes JSON inheritance, and manages G-code injection.
            ...
            if "No such file or directory" in err_msg:
                ...
                if match:
                    ...
                    if not os.path.exists(missing_dir):
                        print(f"⚠️  Tool blocked by complex path. Auto-creating directory: {os.path.basename(missing_dir)}")
                        os.makedirs(missing_dir, exist_ok=True)
                        attempts += 1
                        continue

            print(f"❌ Generator module crashed.")
...
def sanitize_and_flatten_filenames():
    ...
    """
    print(f"🧹 Sanitizing and flattening filenames in '{OUTPUT_DIR}'...")
    count = 0
    ...
    if count > 0:
        print(f"✅ Flattened and renamed {count} nested files.")
    else:
        print("✅ No nested files found.")

def fix_json_files(gcode_mode="inject"):
    """
    Iterates through generated JSON files to Fix inheritance and Manage G-Code.

    Args:
        gcode_mode (str):
            'inject'   = Replace G-code with M555 S=ID. (Default)
            'preserve' = Keep the original G-code generated by the tool.
            'wipe'     = Remove all G-code.
    """
    ...
    for filename in glob.glob(search_path):
        try:
            ...
            # --- G-Code Logic ---
            if gcode_mode == "wipe":
                # --no-gcode: Remove everything
                new_data["filament_start_gcode"] = []

            elif gcode_mode == "preserve":
                # --no-add-gcode: Keep original (do nothing)
...
def deploy_files(target_slicer, wipe_destination=False, force_all_users=False):
    ...
    if not os.path.exists(base_user_path):
        print(f"⚠️ Error: Target user directory does not exist: {base_user_path}")
        return
    ...
    if force_all_users:
        print(f"👥 Mode enabled: Scanning ALL user directories in {base_user_path}...")
        try:
            subdirs = [d for d in os.listdir(base_user_path) if os.path.isdir(os.path.join(base_user_path, d))]
            target_dirs = subdirs
            if not target_dirs:
                print("   ⚠️ No user directories found.")
        except Exception as e:
    ...
    for user_dir in target_dirs:
        ...
        if not os.path.exists(dest_filament_path):
            if not force_all_users:
                print(f"⚠️ Target folder does not exist: {dest_filament_path}")
                continue
            else:
                try:
        ...
        print(f"📦 Deploying to user '{user_dir}'...")

        if wipe_destination:
            print(f"   🧹 Wiping existing profiles...")
            existing_files = glob.glob(os.path.join(dest_filament_path, "*.json"))
            for f in existing_files:
                try:
                    os.remove(f)
                except Exception as e:
                    print(f"   ⚠️ Failed to delete {os.path.basename(f)}: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-gcode", action="store_true", help="REMOVE all filament start G-code")
    parser.add_argument("--no-add-gcode", action="store_true", help="KEEP original G-code (do not inject custom commands)")
    ...
    # Determine G-code mode
    current_gcode_mode = "inject"
    if args.no_gcode:
        current_gcode_mode = "wipe"
    elif args.no_add_gcode:
        current_gcode_mode = "preserve"

    fix_json_files(gcode_mode=current_gcode_mode)
