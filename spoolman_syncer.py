...
NEW_COMMENT = "Generated by spoolman2slicer and edited for Orca and BambuLab by Nebulino"
...
def check_and_setup_tool():
    ...
    if not tool_exists or not venv_exists:
        if not tool_exists:
            print(f"⚠️  Tool folder '{TOOL_DIR}' is missing.")
        if not venv_exists:
        ...
        if choice in ["", "y", "yes"]:
            ...
            print("❌ Cannot proceed without setup. Exiting.")
            sys.exit(1)
    else:
...
def setup_tool(tool_exists, venv_exists):
    ...
    if not tool_exists:
        zip_name = "repo.zip"
        print(f"⬇️  Downloading from GitHub ({REPO_URL})...")
        try:
...
def construct_and_validate_url(ip, port):
    ...
    print(f"🔎 Validating connection to Spoolman ({target_host})...")
...
def run_spoolman_generator(valid_url):
    """Executes the module using the VENV python."""
    print(f"🚀 Executing module from {TOOL_DIR} using venv...")
    
    venv_python = get_venv_python()
    
    # Command: venv/bin/python -m spoolman2slicer ...
    cmd = [
    ...
    # Add TOOL_DIR to PYTHONPATH so the venv python can find the downloaded module
    env = os.environ.copy()
...
def fix_json_files():
    ...
    print(f"✅ Fixed {count} JSON files.")

def deploy_files(target_slicer):
    """Copies files to the slicer folder."""
    dest_path = get_slicer_path(target_slicer)
    if not dest_path: 
        print("❌ Unknown slicer.")
        return
    
    if not os.path.exists(dest_path):
        print(f"⚠️ Target folder does not exist: {dest_path}")
        return

    print(f"📦 Deploying to {target_slicer}...")
    count = 0
    for filename in glob.glob("*.json"):
        shutil.copy(filename, os.path.join(dest_path, filename))
        count += 1
    print(f"🎉 Success! Copied {count} profiles.")

# --- Main Execution ---
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sync Spoolman to Slicer")
    ...
    if args.apply:
        ...
        deploy_files(slicer_map[args.apply])
    else:
        print("ℹ️  Run with --apply [orca|bambu] to install automatically.")