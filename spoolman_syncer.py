...
Spoolman Syncer
----------------
An automation wrapper for the 'spoolman2slicer' tool.
This script manages the environment setup, dependency installation,
source code patching (to fix crashes), and post-processing.
    ...
    if level == "spool":
        ...
        if os.path.exists(OUTPUT_DIR):
            try:
                ...
                print("   ✅ Deleted 'spools' directory.")
            except Exception as e:
                print(f"   ❌ Failed to delete spools: {e}")

    elif level == "all":
        print("🧹 Performing full cleanup (Factory Reset)...")
        ...
        print("✨ Cleanup complete. Run the script again to re-initialize.")
        sys.exit(0)
...
def ensure_dependencies_updated():
    ...
    if current_hash != stored_hash:
        print("📦 Dependencies changed or missing. Updating environment...")
...
def patch_tool_code():
    """
    HOTFIX: Scans the downloaded tool source code and injects filename sanitization.
    This prevents crashes when Spoolman data contains slashes (e.g. 'PLA+/Pro').
    """
    print("🩹 Patching generator source code to handle special characters...")
    patch_count = 0

    # We walk through all python files in the tool directory
        ...
        for file in files:
            if file.endswith(".py"):
                ...
                try:
                    ...
                    # Heuristic Patch:
                    # Look for os.path.join where a filename variable is used, and force a replace on it.
                    # We look for lines that typically construct the output path.

                    # Original problematic pattern in many tools: os.path.join(directory, filename)
                    # We replace it with a version that sanitizes the second argument.

                    # Specific target: We want to catch the variable that holds the json filename.
                    if "os.path.join" in content and ".json" in content:
                        # This is a bit brute force, but effective for this specific crash.
                        # We inject a sanitize call on any variable that looks like it's being joined.

                        # Fix 1: Generic replacement for variables named 'filename' or 'file_name'
                        # Replacing: os.path.join(..., filename)
                        # With:      os.path.join(..., filename.replace('/', '-').replace('\\', '-'))

                        new_content = re.sub(
                            r'os\.path\.join\(([^,]+),\s*([a-zA-Z0-9_]+)\)',
                            r'os.path.join(\1, \2.replace("/", "-").replace("\\\\", "-"))',
                            content
                        )
    ...
    if patch_count > 0:
        print(f"✅ Applied patches to {patch_count} files.")
    else:
        print("ℹ️  No patches needed or code structure changed.")
...
def check_and_setup_tool():
    ...
    if not tool_exists or not venv_exists:
        if not tool_exists:
            print(f"⚠️  Generator tool '{TOOL_DIR}' is missing.")
        if not venv_exists:
        ...
        if choice in ["", "y", "yes"]:
            ...
            print("❌ Setup required to proceed. Exiting.")
            sys.exit(1)
    else:
        print(f"✅ Environment verified.")

    # Always ensure dependencies match
    ensure_dependencies_updated()

    # Always run the patcher to ensure code is safe (idempotent)
    patch_tool_code()
...
def setup_tool(tool_exists, venv_exists):
    ...
    if not tool_exists:
        zip_name = "repo.zip"
        print(f"⬇️  Downloading upstream repository ({REPO_URL})...")
        try:
            ...
            print(f"✅ Installed tool to: {TOOL_DIR}")
        except Exception as e:
...
def construct_and_validate_url(ip, port):
    ...
    print(f"🔎 Connecting to Spoolman at {target_host}...")

    try:
        with urllib.request.urlopen(target_host, timeout=3) as response:
            if response.status != 200:
                print(f"⚠️ Server responded with status code: {response.status}")
    except Exception as e:
        print(f"❌ Connection Failed: Could not reach Spoolman at {target_host}")
...
def run_spoolman_generator(valid_url):
    ...
    try:
        ...
        print(f"❌ Generator module failed (likely due to bad data or network).")
        sys.exit(1)

def sanitize_filenames():
    """
    Scans the output directory for unsafe characters in filenames (like / or \)
    and renames them. This handles cases where the patch didn't catch everything.
    """
    print(f"🧹 Verifying filenames in '{OUTPUT_DIR}'...")
    count = 0
...
def deploy_files(target_slicer, wipe_destination=False):
    ...
    if not dest_path:
        print("❌ Error: Unsupported slicer selected.")
        return

    if not os.path.exists(dest_path):
        print(f"⚠️ Error: Target directory does not exist: {dest_path}")
        return

    if wipe_destination:
        print(f"🧹 Wiping existing profiles in {target_slicer} directory...")
    ...
    for filename in glob.glob(search_path):
        ...
        count += 1
    print(f"🎉 Success! Deployed {count} profiles.")
...
if __name__ == "__main__":
    ...
    if args.apply:
        ...
        print("ℹ️  Files generated in 'spools/'. Run with --apply [orca|bambu] to install automatically.")